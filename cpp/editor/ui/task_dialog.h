...
// Plain data holder, not a Wrapped-derived class: AcceptDialog is already a registered engine class, so this needs no
// GDCLASS/GDREGISTER_INTERNAL_CLASS at all. Pairs the AcceptDialog with the specific children
// make_appear()/update_state()/stop() below need direct access to.
namespace godot {
    ...
    inline void on_task_dialog_scroll_container_draw(ScrollContainer* scroll_container) {
    ...
    inline void task_dialog_update_state(const TaskDialogControls& controls, String log) {
