...
void GodotPrintBridge::print_exception_stacktrace(jni::Env p_env, jni::JThrowable p_throwable) {
    ...
    jni::JString str = jni::JString(get_instance().wrapped.call_object_method(p_env, GET_EXCEPTION_STACKTRACE, args));
...
godot::String GodotPrintBridge::get_jvm_stacktrace(jni::Env& p_env) {
    jni::JString str = jni::JString(wrapped.call_object_method(p_env, GET_STACKTRACE));
