...
namespace godot {
    ...
    static StringName get_gdj_fq_name(const String& source) {
    ...
    static _FORCE_INLINE_ bool is_end_of_source(const String& source, int index) {
        return index >= source.length();
    }

    static bool is_identifier_character(char32_t character) {
    ...
    static bool skip_non_code_once(const String& source, int& index) {
        ...
        if (next_character != U'/' || index >= source.length() - 1) { return false; }
        ...
        if (next_character != U'*') { return false; }
    ...
    static void skip_non_code(const String& source, int& index) {
        while (!is_end_of_source(source, index) && skip_non_code_once(source, index)) {}
    }

    static bool matches_keyword_at(const String& source, int index, const String& keyword) {
        const int keyword_size = keyword.length();
        if (index < 0 || index + keyword_size > source.length()) { return false; }

        for (int i = 0; i < keyword_size; ++i) {
            if (source[index + i] != keyword[i]) { return false; }
        }

        const bool has_previous_character = index > 0;
        if (has_previous_character && is_identifier_character(source[index - 1])) { return false; }

        const int next_index = index + keyword_size;
        const bool has_next_character = next_index < source.length();
        if (has_next_character && is_identifier_character(source[next_index])) { return false; }

        return true;
    }

    static int find_token(const String& source, int start_index, const String& token, bool exact_word) {
        while (!is_end_of_source(source, start_index)) {
            skip_non_code(source, start_index);
            if (is_end_of_source(source, start_index)) { return -1; }

            if (exact_word) {
                if (matches_keyword_at(source, start_index, token)) { return start_index; }
    ...
    static String get_package_name(const String& source, int& r_register_class_search_start) {
    ...
    static void skip_annotation_arguments(
        const String& source,
        int annotation_index,
        const String& annotation,
        int& r_class_search_index
    ) {
    ...
    static String find_class_name(const String& source, int search_start, const String& expected_name = String()) {
