...
- Check that the IntelliJ **Settings > Godot-JVM > Annotation processing mode** setting matches `registration.annotationProcessingMode` in `build.gradle.kts`
...
**Explanation:** The editor looks for a JVM in this order: an explicit `--jvm-path`, the embedded JRE under `jvm/`, `JAVA_HOME`, the first usable `java` on `PATH`, then on macOS the JDK reported by `/usr/libexec/java_home -v 17+`. Exported games use the embedded JRE unless `--jvm-path` overrides it. An invalid explicit path fails startup without fallback.
...
### The export dialog reports a missing JVM build

**Symptom:** the export dialog lists `JVM build does not exist at res://jvm/release/game.jar` (or the debug
pair, a native image, a dex JAR or an iOS archive) as an error and the export completes with errors. A
headless export prints the same line but still exits successfully, so a CI script has to check the output.
