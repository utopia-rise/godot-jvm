...
If they do, the JDK is on your `PATH`. The editor can use it when neither an embedded JRE nor a usable `JAVA_HOME` takes priority.

If a command is not found, or reports an older version, your installer did not update `PATH` or another Java shadows the new one. Either fix `PATH`, or set `JAVA_HOME` to the JDK's installation directory. The editor checks `JAVA_HOME` before `PATH`.

!!! note "Which JDK the editor uses"
    The editor looks for a JVM in this order: an explicit `--jvm-path`, the embedded JRE under `jvm/`, `JAVA_HOME`, the first usable `java` on `PATH`, then on macOS the JDK reported by `/usr/libexec/java_home -v 17+`. Exported games use the embedded JRE unless `--jvm-path` overrides it. An invalid explicit path fails startup without fallback.
