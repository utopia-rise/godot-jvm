...
func test_dependencies_of_every_configuration_are_loadable() -> void:
    ...
    assert_that(dependency_test.single_jar_dependency()).override_failure_message("godotExternalImplementation dependencies should load from jvm/<variant>/dependencies/ next to the user code jar").is_equal("jvm")
