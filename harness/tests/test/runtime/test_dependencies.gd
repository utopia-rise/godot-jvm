...
func test_dependencies_of_every_configuration_are_loadable() -> void:
    ...
    assert_that(dependency_test.main_jar_dependency()).override_failure_message("godotMain dependencies should load from main.jar").is_equal("Godot")
    assert_that(dependency_test.single_jar_dependency()).override_failure_message("godotSingle dependencies should load from jvm/external/ next to main.jar").is_equal("jvm")
